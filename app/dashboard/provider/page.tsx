...
export default async function ProviderDashboard() {
  ...
  const { data: rawProfile } = await supabaseAdmin
    ...
    .select('id, bio, ic_verified, license_verified, rating_avg, users!user_id(full_name, avatar_url)')
    .eq('user_id', user.id)
    .maybeSingle()
  ...
  const profile = {
    id: rawProfile.id,
    bio: rawProfile.bio,
    icVerified: rawProfile.ic_verified,
    licenseVerified: rawProfile.license_verified,
    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-16 md:pb-0">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              {profile.licenseVerified ? (
                <span className="flex items-center gap-1 text-xs text-teal-700 font-medium">
                  <CheckCircle className="w-3.5 h-3.5" /> Sijil Profesional Disahkan
                </span>
              ) : profile.icVerified ? (
                ...
                <span className="flex items-center gap-1 text-xs text-yellow-600 font-medium">
                  <Clock className="w-3.5 h-3.5" /> Menunggu Pengesahan
                </span>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard icon={DollarSign} label={dc.monthlyEarnings} value={`RM${monthlyEarnings.toFixed(0)}`} color="text-teal-700 bg-teal-100" />
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/dashboard/provider/bookings" className="text-sm text-teal-600 font-medium hover:underline">{dc.viewAll}</Link>
            </div>
...
function QuickAction({ href, icon: Icon, label, desc }: { href: string; icon: typeof Star; label: string; desc: string }) {
  return (
    <Link href={href}
      className="flex items-center gap-3 bg-white rounded-xl border border-gray-100 p-4 hover:border-teal-500 hover:shadow-sm transition-all group">
      <div className="w-9 h-9 rounded-lg bg-teal-50 text-teal-600 flex items-center justify-center flex-shrink-0 group-hover:bg-teal-600 group-hover:text-white transition-colors">
        <Icon className="w-5 h-5" />
      </div>
      <div>
