  ...
  return (
    <div className="flex items-center gap-1">
      <button onClick={() => setShowPhotos(!showPhotos)}
        className="flex items-center gap-1 text-xs text-teal-700 bg-teal-50 px-2 py-1 rounded-full hover:bg-teal-100 transition-colors">
        <Eye className="w-3 h-3" /> Review IC
      </button>
      ...
      {showPhotos && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setShowPhotos(false)}>
          <div className="bg-white rounded-2xl p-5 max-w-lg w-full space-y-4 shadow-2xl" onClick={e => e.stopPropagation()}>
            <h3 className="font-semibold text-gray-900">IC Review — {icNumber}</h3>
            <div className="grid grid-cols-2 gap-3">
              {icFrontUrl && (
                <div>
                  <p className="text-xs text-gray-500 mb-1">IC Front</p>
                  <img src={icFrontUrl} alt="IC Front" className="w-full rounded-xl border border-gray-200 object-cover" />
                </div>
              )}
              {icBackUrl && (
                <div>
                  <p className="text-xs text-gray-500 mb-1">IC Back</p>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => act('reject')} disabled={!!loading}
                    ...
                    {loading === 'reject' ? <Loader2 className="w-4 h-4 animate-spin" /> : null} Submit Rejection
                  </button>
                  <button onClick={() => setShowReject(false)} className="px-3 py-2 text-sm bg-gray-100 rounded-lg">Cancel</button>
                </div>
              </div>
            )}
          </div>
