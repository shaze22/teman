    ...
    <div className="p-8">
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{bookings.length} bookings · RM{totalRevenue.toFixed(0)} platform revenue</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <div className="divide-y divide-gray-50">
          {bookings.map(b => (
            <Link key={b.id} href={`/booking/${b.id}`} className="grid grid-cols-[auto_1fr_1fr_auto_auto_auto_auto] gap-0 px-4 py-3 items-center hover:bg-teal-50 transition-colors cursor-pointer">
              <div className="w-28 text-xs font-mono text-teal-600">{b.bookingCode}</div>
              ...
              <div className="w-24 text-center">
                <div className="text-xs text-gray-600">{new Date(b.scheduledDate).toLocaleDateString('en-MY')}</div>
              ...
              <div className="w-20 text-right text-sm font-medium text-teal-600">RM{b.platformFee.toFixed(0)}</div>
              <div className="w-24 text-center">
