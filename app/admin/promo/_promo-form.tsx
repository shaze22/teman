...
export default function PromoForm() {
  ...
  const inp = 'w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400'

  return (
    <div>
      {!show ? (
        <button onClick={() => setShow(true)} className="flex items-center gap-2 px-4 py-2 bg-[#0D9488] text-white text-sm font-semibold rounded-xl hover:bg-teal-700 transition-colors">
        ...
        <form onSubmit={submit} className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4 mb-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className={inp} value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} placeholder="SENIO10" required />
            </div>
            <div>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Value {form.discount_type === 'percentage' ? '(%)' : '(RM)'}</label>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Min. Purchase (RM)</label>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Max Uses (empty = unlimited)</label>
              <input type="number" className={inp} value={form.max_uses} onChange={e => setForm(f => ({ ...f, max_uses: e.target.value }))} placeholder="100" min={1} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Expires On</label>
              <input type="date" className={inp} value={form.valid_until} onChange={e => setForm(f => ({ ...f, valid_until: e.target.value }))} />
            </div>
          </div>
          <div className="flex gap-2">
            <button type="submit" disabled={loading} className="flex-1 flex items-center justify-center gap-2 bg-[#0D9488] text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-teal-700 disabled:opacity-50">
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : null} Save
            </button>
            <button type="button" onClick={() => setShow(false)} className="px-4 py-2.5 text-sm text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200">Cancel</button>
          </div>
        </form>
      )}
    </div>
