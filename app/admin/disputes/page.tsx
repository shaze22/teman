  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">{open.length} active · {closed.length} resolved</p>
      </div>

      {open.length > 0 && (
        <section className="mb-8">
          <h2 className="text-sm font-semibold text-red-600 uppercase tracking-wider mb-3">Requires Action ({open.length})</h2>
      ...
      <section>
        ...
        <div className="space-y-3">
          ...
          {closed.length === 0 && <p className="text-sm text-gray-400">None yet.</p>}
        </div>
      </section>
    </div>
  )
      ...
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <Link href={`/booking/${d.booking_id}`} className="text-xs font-mono text-teal-600 hover:underline">
              #{d.bookingCode}
            </Link>
            <span className="text-xs text-gray-400">RM{d.totalAmount.toFixed(0)}</span>
          </div>
          <p className="text-sm font-medium text-gray-900">By: {d.raiserName}</p>
          <p className="text-xs text-gray-500 mt-0.5">{new Date(d.created_at).toLocaleDateString('en-MY', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
        </div>
      </div>
