...
const NEW_PROVIDER_EMAILS = [
  'aishah.rahman@teman.my',
  'nurulain.zainuddin@teman.my',
  'faridah.yusof@teman.my',
  'norhayati.kamaruddin@teman.my',
  'sitizubaidah.hashim@teman.my',
  'rosnah.ibrahim@teman.my',
  'norazlina.mustafa@teman.my',
  'zuraidah.othman@teman.my',
  'nabilah.aziz@teman.my',
  'hafizah.ismail@teman.my',
]
...
// Fetch only the 10 new providers
const { data: allProviders } = await supabase
  .from('single_mother_profiles')
  .select('id, user_id, users!inner(full_name, email), provider_pricing(service_type, price)')
  .eq('is_active', true)
  .in('users.email', NEW_PROVIDER_EMAILS)

// Filter in JS since Supabase .in on joined table may not filter rows
const providers = (allProviders ?? []).filter(p =>
  NEW_PROVIDER_EMAILS.includes(p.users?.email)
)
...
console.log(`Found ${providers.length} new providers, ${customers.length} customers`)
if (providers.length === 0) { console.error('No providers found — check emails'); process.exit(1) }
...
for (const provider of providers) {
  ...
  const getPrice = (type) => {
    ...
    return p ? parseFloat(p.price) : 40
  }
...
// Patch balance_after with running total per provider
const txByProvider = {}
...
for (const provider of providers) {
  ...
  const providerReviews = allReviews.filter(r => r.reviewee_id === provider.user_id)
...
console.log('Earnings per provider:')
for (const provider of providers) {
